...
nlohmann::json GetPara(std::string para_name) {
 ...
 // nlohmann::json data;
  ...
  minilog_debug(" dbserver_system_get str is %s:", str);
 ...
 // char *content1;
  if (!para_name.compare("screenshot")) {
    ...
    minilog_debug(" dbserver_system_para_get_by_name is %s:", str);
    data = nlohmann::json::parse(str).at("jData");
    minilog_debug(" data is %s:", str);
    if (!data.empty()) {
       content = data.at(0).at(para_key);
     }
  }
  minilog_debug(" data.at(0).at(para_key)).dump().c_str() %s:",content.dump().c_str());
  return content;
}

void SystemApiHandler::handler(const HttpRequest &Req, HttpResponse &Resp) {
  minilog_debug("system systemsystemsystemsystemsystemsystemsystemsystemsystem system system");
  char *str;
  ...
  if (Req.Method == "GET") {
    if (!path_specific_resource.compare("device-info")) {
      ...
      minilog_debug("paraparaparapara paraparaparapara paraparapara");
      if (para_channel.empty()) {
        Resp.setErrorResponse(HttpStatus::kNotImplemented, "Not Implemented");
      } else {
        minilog_debug("111111111111111111111111para_channel:%s,%s, %s", path_api_resource.c_str(),path_specific_resource.c_str(),para_channel.c_str());
        //char content_str[40000];
       /// memset(content_str, '\0', 40000);
    ...
    if (!path_specific_resource.compare("device-info")) {
      ...
      minilog_debug("path is loginpath is loginpath is loginpath is loginpath is login");
        std::string username = system_config.at("sUserName");
         std::string sPassword = system_config.at("sPassword");
       minilog_debug("username is :%s\n",username.c_str());
       minilog_debug("username is :%s\n",sPassword.c_str());
        ...
        if (status >= 0) {
          ...
          FILE *pFile=fopen("../Cookies.txt","w+");
          if(pFile !=NULL)
          {
              fwrite(token.c_str(), token.length(), 1, pFile);  
              fclose(pFile);   
          } else{minilog_debug("fopen Cookies.txt  to write failed ");}
       ...
       minilog_debug("setErrorResponsesetErrorResponsesetErrorResponsesetErrorResponsesetErrorResponse");
      Resp.setErrorResponse(HttpStatus::kBadRequest, "Not Implemented");
    }
