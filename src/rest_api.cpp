...
std::string checkToken(HttpRequest &Req) {
  ...
  {
    minilog_debug("Req.PathInfo.compare(/system/logi && Req.PathInfo.find(system/para/webPage");
    char buffer_cookies[1024];
    FILE *pFile=fopen("../Cookies.txt","r+");
    if(pFile !=NULL)
    {
         while (!feof(pFile)) // to read file 
         { 
             fread(buffer_cookies, sizeof(buffer_cookies), 1, pFile); 
          }  
         Req.Cookies = buffer_cookies;
         Req.Cookies = "token=" + Req.Cookies.substr(0, Req.Cookies.length()-2);
    }else{
     minilog_debug("fopen Cookies.txt to read failed ");
     }
...
HttpStatus parseRequest(HttpRequest &Req) {
  ...
  //minilog_debug(getenv("QUERY_STRING"));
  //minilog_debug(getenv("CONTENT_LENGTH:"));
  //minilog_debug(getenv("AUTH_TYPE"));
  //minilog_debug(getenv("SCRIPT_NAME"));
 // minilog_debug(getenv("SERVER_NAME"));
 // minilog_debug(getenv("REMOTE_ADDR"));
 // minilog_debug(getenv("PATH_INFO"));
 // minilog_debug(getenv("SERVER_SOFTWARE"));
//  minilog_debug(getenv("SERVER_PORT"));
//  minilog_debug(getenv("GATEWAY_INTERFACE"));
//  minilog_debug(getenv("SERVER_PROTOCOL"));
//  minilog_debug(getenv("REQUEST_METHOD"));
 // minilog_debug(getenv("HTTP_ACCEPT"));
 // minilog_debug(getenv("HTTP_USER_AGENT"));
 // Req.PathInfo = getenv("PATH_INFO");
