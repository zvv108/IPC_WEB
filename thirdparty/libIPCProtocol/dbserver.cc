...
char *dbserver_cmd(char *json, char *interface)   //interface : "rockchip.dbserver.system"
{
//    printf("json  is : %s \n",json);
...
//    printf("table is : %s \n",table);
    ...
    json_object_object_add(j_cfg, "data", json_object_new_string("*"));
    json_object_object_add(j_cfg, "cmd", json_object_new_string("Select"));
//    printf("key is : %s \n",(char *)json_object_to_json_string(key));

//    printf("j_cfg is : %s \n",(char *)json_object_to_json_string(j_cfg));
    json_config = (char *)json_object_to_json_string(j_cfg);  //将struct json_object类型的json数据转化为字符串

//    printf("json_config is : %s \n",json_config);
