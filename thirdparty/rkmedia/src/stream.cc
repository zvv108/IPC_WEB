...
bool Stream::ReadImage(void *ptr, const ImageInfo &info) {
  ...
  if ((info.pix_fmt != PIX_FMT_FBC0) && (info.pix_fmt != PIX_FMT_FBC2) &&
    ...
    if (ret != read_size) {
      // RKMEDIA_LOGI("read ori stream failed, %d != %d\n", ret, read_size);
      return false;
    }
    return true;
  }
  ...
  case PIX_FMT_NV12:
    for (row = 0; row < info.height; row++) {
      ...
      if ((int)read_size != info.width) {
        // RKMEDIA_LOGI("read ori yuv stream luma failed, %d != %d\n",
    ...
    for (row = 0; row < info.height / 2; row++) {
      ...
      if ((int)read_size != info.width) {
        // RKMEDIA_LOGI("read ori yuv stream cb failed, %d != %d\n", read_size,
        // info.width);
        return false;
      }
    }
  ...
  default:
    RKMEDIA_LOGI("TODO: read image fmt %d\n", info.pix_fmt);
    return false;
  }
  return true;
}
