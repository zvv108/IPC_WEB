...
int system_dbus_register(DBusConnection *dbus_conn) {
  bool ret = g_dbus_register_interface(dbus_conn, "/", DB_SYSTEM_INTERFACE, methods,
                            signals, NULL, DB_SYSTEM_INTERFACE, NULL);
  printf("system_dbus_register ret is :%d,\n",ret);
  return 0;
}
